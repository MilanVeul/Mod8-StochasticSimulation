...
class StatisticHolder:
    ...
    def report(self):
        stats = {}
        stats[StatisticHolder.AVG_QUEUE_LEN] = self.stat_queue_len.mean(self.model.sim.current_time)
        stats[StatisticHolder.AVG_WAITING_TIME] = self.stat_sojourn_time.mean()
        stats[StatisticHolder.TRUCK_UTILISATION] = [util.mean(self.model.sim.current_time) for util in self.stat_truck_util]
